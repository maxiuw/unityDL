...
public class ImageSynthesis : MonoBehaviour {
	...
	void Start()
	{
		...
		Debug.Log(capturePasses[0].camera);
		// Debug.Log("Cam is ok" + capturePasses[0].camera.targetTexture.width + capturePasses[0].camera.targetTexture.height);
	...
	public void Save(string filename, int width = -1, int height = -1, string path = "", int channel = -1)
	{
		if (width <= 0 || height <= 0)
		{
			width = Screen.width;
		...
		filenameWithoutExtension = Path.Combine(path, filename);
		Debug.Log(path);
		// execute as coroutine to wait for the EndOfFrame before starting capture
		StartCoroutine(
			WaitForEndOfFrameAndSave(filenameWithoutExtension, filenameExtension, width, height, channel));
	}

	private IEnumerator WaitForEndOfFrameAndSave(string filenameWithoutExtension, string filenameExtension, int width, int height, int channel)
	{
		yield return new WaitForEndOfFrame();
		Save(filenameWithoutExtension, filenameExtension, width, height, channel);
	}

	private void Save(string filenameWithoutExtension, string filenameExtension, int width, int height, int channel) 
	{	
		if (channel == -1) {
			foreach (var pass in capturePasses)
			...
			// filenameWithoutExtension = "/input/" + filenameWithoutExtension;
			Save(pass.camera, filenameWithoutExtension + pass.name + filenameExtension, width, height, pass.supportsAntialiasing, pass.needsRescale);
			pass = capturePasses[channel];
			// filenameWithoutExtension = "/output/" + filenameWithoutExtension;
			Save(pass.camera, filenameWithoutExtension + pass.name + filenameExtension, width, height, pass.supportsAntialiasing, pass.needsRescale);
		}
